...
function restricted(req, res, next) {
  const password = req.headers.authorization;
    ...
    next({ message: 'No authorization header provider' })
  }
}
...
function only(name) {
  ...
  return function(req, res, next) {
    const personName = req.headers.name || ''; // just in case there is no name header provider

    // This function can see the  parameter passed to the wrapper function
